...
class PoseEstimationConfigWidget(IConfigWidget):
    def __init__(self, app_context: IAppContext, config: PoseEstimationConfig, parent=None):
        ...
        self.frm_pose_model = QWidget(self)
        frm_pose_model_layout = QHBoxLayout(self.frm_pose_model)
        frm_pose_model_layout.setContentsMargins(0, 0, 0, 0)

        self.dpd_pose_model = QComboBox(self.frm_pose_model)
        ...
        for model_config in pose_estimation_models_settings.models.values():
            self.dpd_pose_model.addItem(model_config.name, model_config.id)
        self.dpd_pose_model.setCurrentIndex(self.dpd_pose_model.findData(self.config.pose_model_id))
        frm_pose_model_layout.addWidget(self.dpd_pose_model)

        self.btn_edit_pose_models = QToolButton(self.frm_pose_model)
        self.btn_edit_pose_models.setText("...")
        frm_pose_model_layout.addWidget(self.btn_edit_pose_models)

        self.layout = QFormLayout(self)
        self.layout.addRow("Video File", self.txt_video_file)
        self.layout.addRow("Pose Results File", self.txt_pose_results_file)
        self.layout.addRow("Pose Model", self.frm_pose_model)
        self.setLayout(self.layout)

        self.btn_edit_pose_models.clicked.connect(self._edit_pose_models)
        ...
        self.dpd_pose_model.currentIndexChanged.connect(self._pose_model_changed)

    def get_config(self) -> PoseEstimationConfig:
        return self.config
    ...
    def _pose_model_changed(self):
        model_id = self.dpd_pose_model.itemData(self.dpd_pose_model.currentIndex())
