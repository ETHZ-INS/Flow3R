...
class PoseResultsSink(Observer):
    """
    RxPY Observer that writes pose results to a CSV file.

    - Exposes a single Future `result`:
        * set_result(Path) on success (on_completed)
        * set_exception(exc) on error
        * set_exception(CancelledError) if disposed before completion
    ...
    def dispose(self):
        """Early stop: unsubscribe, cleanup, and mark as cancelled."""
        self._unsubscribe()
        self._cleanup_once()
        with self._lock:
            if not self.result.done():
                self.result.set_exception(CancelledError("PoseResultsSink disposed"))
