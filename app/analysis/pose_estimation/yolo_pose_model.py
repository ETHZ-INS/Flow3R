...
def parse_instance_types(instance_types: List[dict]) -> List[InstanceType]:
    ...
    for instance_type in instance_types:
        ...
        instance_type = InstanceType(name, point_names, skeleton)
        parsed_instance_types.append(instance_type)
    return parsed_instance_types


class YoloPoseModel(PoseModel):
    def __init__(self, model: YOLO, instance_types: List[InstanceType], output_mapping: dict, device: str = "cuda", confidence_threshold: float = 0.1, iou_threshold: float = 0.1, input_channel_mode: str = "color"):
        self.model = model
    ...
    def from_folder(cls, folder: Path, device: str = "cuda", confidence_threshold: float = 0.1, iou_threshold: float = 0.1):
        ...
        config = cls.load_model_config(folder)
        input_channel_mode = config.get("input_channel_mode", "color")
        model_weights_file = folder / "weights" / "best.pt"
        return cls(YOLO(str(model_weights_file)), instance_types, output_mapping, device, confidence_threshold, iou_threshold, input_channel_mode)

    @classmethod
    def load_instance_types(cls, model_folder: Path) -> List[InstanceType]:
    ...
    @classmethod
    def load_model_config(cls, model_folder: Path) -> dict:
        config_file = model_folder / "meta" / "model.yaml"
    ...
    def parse_result(self, result) -> List[Instance]:
        ...
        for instance_index, instance_conf in enumerate(instance_confidences):
            ...
            for point_name in instance_type.point_names:
                ...
                point_coords.append(result.keypoints.xy[instance_index][output_index])

            points = [Point(float(coords[0]), float(coords[1]), float(conf)) for conf, coords in zip(point_confs, point_coords)]

            instances.append(Instance(instance_id, instance_type, box_coords, points, float(instance_conf)))

        return instances

    def predict_frame(self, frame: np.ndarray) -> List[Instance]:
        results = self.model(frame, verbose=False, device=self.device, conf=self.confidence_threshold, iou=self.iou_threshold)
        return self.parse_result(results[0])

    def predict_frames(self, frames: List[np.ndarray]) -> List[List[Instance]]:
        results = self.model(frames, verbose=False, device=self.device, conf=self.confidence_threshold, iou=self.iou_threshold)
        return [self.parse_result(result) for result in results]
